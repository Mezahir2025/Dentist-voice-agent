  ...
  if (role === 'patient') {
    return (
      <div className="min-h-screen bg-white font-['Plus_Jakarta_Sans',sans-serif] selection:bg-emerald-100 selection:text-emerald-900">
        ...
        <header className="fixed top-0 w-full z-50 px-4 py-6 transition-all duration-300">
          <nav className="mx-auto max-w-7xl rounded-full backdrop-blur-xl border shadow-lg px-6 py-3 flex items-center justify-between transition-all duration-300 bg-white/80 border-white/20 shadow-slate-200/50">
            ...
            <div className="hidden lg:flex items-center gap-3">
              <button onClick={() => setIsAuthModalOpen(true)} className={`inline-flex items-center gap-2 border border-${BRAND_CONFIG.colors.primary}-500 ${BRAND_CONFIG.colors.primaryText} text-sm font-medium px-4 xl:px-5 py-2.5 rounded-full transition-all hover:bg-emerald-50 active:scale-95`}>
                <LogIn className="w-4 h-4" /> Həkim Girişi
              </button>
              <button onClick={() => setIsVoiceWidgetOpen(true)} className={`inline-flex items-center gap-2 ${BRAND_CONFIG.colors.primaryFull} text-sm font-medium px-4 xl:px-5 py-2.5 rounded-full transition-all shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/40 ${BRAND_CONFIG.colors.primaryHover} text-white active:scale-95`}>
                Randevu Al
          ...
          <div className={`fixed inset-0 z-[60] bg-white transition-all duration-300 lg:hidden ${isMobileMenuOpen ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0 pointer-events-none'}`}>
            <div className="p-6 h-full flex flex-col">
              ...
              <div className="mt-auto space-y-4">
                <button onClick={() => { setIsAuthModalOpen(true); setIsMobileMenuOpen(false); }} className={`w-full flex items-center justify-center gap-2 border-2 border-${BRAND_CONFIG.colors.primary}-500 ${BRAND_CONFIG.colors.primaryText} font-bold py-4 rounded-2xl transition-all active:scale-95`}>
                  <LogIn className="w-5 h-5" /> Həkim Girişi
                </button>
                <button onClick={() => { setIsVoiceWidgetOpen(true); setIsMobileMenuOpen(false); }} className={`w-full flex items-center justify-center gap-2 ${BRAND_CONFIG.colors.primaryFull} text-white font-bold py-4 rounded-2xl shadow-xl shadow-emerald-500/20 active:scale-95`}>
            ...
            <div className="grid lg:grid-cols-2 gap-12 lg:gap-24 items-center">
              ...
              <div className="space-y-8">
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <button onClick={() => setIsVoiceWidgetOpen(true)} className={`flex items-center justify-center gap-2 ${BRAND_CONFIG.colors.primaryFull} text-white px-8 py-5 rounded-2xl font-bold text-lg shadow-2xl shadow-emerald-500/30 ${BRAND_CONFIG.colors.primaryHover} transition-all active:scale-95`}>
                    Randevu Al <ArrowRight className="w-5 h-5" />
                  </button>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
              ...
              <div>
                ...
                <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); setIsVoiceWidgetOpen(true); }}>
                  ...
                  <button type="button" onClick={() => setIsVoiceWidgetOpen(true)} className={`w-full ${BRAND_CONFIG.colors.primaryFull} font-semibold text-lg py-4 rounded-xl transition-all shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/40 flex items-center justify-center gap-2 mt-4 ${BRAND_CONFIG.colors.primaryHover} text-white`}>
                    Randevu Al <ArrowRight className="w-5 h-5" />
                  </button>
                </form>
              </div>
        ...
        <Auth isOpen={isAuthModalOpen} onClose={() => setIsAuthModalOpen(false)} onAuthSuccess={(u) => { setCurrentUser(u); setRole('dentist'); }} />
      </div>
    );
  }
